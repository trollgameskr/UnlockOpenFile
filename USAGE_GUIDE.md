...
**설정 방법:**
1. 설정 창을 엽니다 (인자 없이 `UnlockOpenFile.exe` 실행)
2. "파일 그룹 관리" 섹션에서 "추가" 버튼을 클릭합니다
3. 그룹 이름을 입력합니다 (예: "프로젝트A 문서")
4. 함께 열릴 파일들을 선택합니다 (여러 파일 선택 가능)
5. 설정이 저장되고 목록에 표시됩니다

**파일 관리:**
1. 목록에서 그룹을 선택합니다
2. "파일 관리" 버튼을 클릭하여 그룹 내 파일을 추가하거나 제거할 수 있습니다
3. "삭제" 버튼으로 그룹 전체를 제거할 수 있습니다

**동작 원리:**
- 그룹에 속한 파일 중 하나를 열면 그룹의 모든 파일이 자동으로 열립니다
- 각 파일은 별도의 임시 복사본으로 관리됩니다
- 설정은 레지스트리 `HKEY_CURRENT_USER\Software\UnlockOpenFile\FileGroups`에 저장됩니다

**사용 예:**
- 데이터 파일(`data.xlsx`)과 분석 결과(`analysis.xlsx`)를 항상 함께 열기
- 프로젝트 관련 여러 문서(기획서, 명세서, 일정표)를 동시에 참조
- 설정 파일(`config.json`)과 메인 파일(`main.xlsx`)을 항상 함께 편집
...
### 시나리오 4: 관련 파일들을 항상 함께 열기
**문제:** 데이터 분석 작업 시 원본 데이터 파일, 분석 결과 파일, 설정 파일을 항상 함께 열어야 합니다.

**해결:**
1. 설정 창에서 "데이터 분석" 그룹을 생성합니다
2. 다음 파일들을 그룹에 추가합니다:
   - `data.csv` (원본 데이터)
   - `analysis.xlsx` (분석 결과)
   - `config.txt` (설정 파일)
3. 이후 `data.csv` 파일을 열면 `analysis.xlsx`와 `config.txt`도 자동으로 함께 열립니다
4. 각 파일은 독립적으로 편집 가능하며 변경 사항이 자동으로 원본에 저장됩니다
