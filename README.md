...
- **파일 그룹 관리**: 함께 열리는 파일들을 그룹으로 관리하여 한 파일을 열면 그룹의 모든 파일이 자동으로 열립니다
...
함께 작업하는 파일들을 그룹으로 관리하여 한 파일을 열 때 관련된 모든 파일을 자동으로 함께 열 수 있습니다:

**설정 방법:**
1. 설정 창을 엽니다 (인자 없이 실행)
2. "파일 그룹 관리" 섹션에서 "추가" 버튼 클릭
3. 그룹 이름 입력 (예: "프로젝트A 문서")
4. 함께 열릴 파일들을 선택 (여러 파일 선택 가능)
5. 설정 완료

**사용 방법:**
- 그룹에 속한 파일 중 하나를 열면 그룹의 모든 파일이 자동으로 열립니다
- "파일 관리" 버튼으로 기존 그룹에 파일을 추가하거나 제거할 수 있습니다
- 그룹이 더 이상 필요없으면 "삭제" 버튼으로 제거할 수 있습니다

**활용 예시:**
- 데이터 파일과 분석 결과를 항상 함께 열어야 하는 경우
- 프로젝트 관련 여러 문서를 동시에 참조해야 하는 경우
- 설정 파일과 메인 파일을 항상 함께 편집하는 경우
